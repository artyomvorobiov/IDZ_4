...
int cab[3] = {-1, -1, -1}; //буфер(кабинеты)
...
// поток-администратор блокируется этой условной переменной,
// когда количество занятых ячеек становится равно bufSize
pthread_cond_t not_full ;

// поток-посетитель блокируется этой условной переменной,
// когда количество занятых кабинетов становится равно 0
pthread_cond_t not_empty ;

//стартовая функция потоков – администраторы (2)
[[noreturn]] void *Producer(void *param) {
    ...
    while (1) {
        ...
        if (cab[front] != -1) {
            ...
            printf("Patient %d: LEFT from cabinet [%d]\n", y, front + 1); // кто-то ушел, админы об этом сообщают и дают возможномть пройти следующему посетителю
            sleep(3);
        }
        //sleep(3);
        //разбудить потоки-посетители после добавления элемента в буфер
        pthread_cond_broadcast(&not_empty) ;
    }
    return NULL;
}

//стартовая функция потоков – клиенты (n)
[[noreturn]] void *Consumer(void *param) {
    ...
    while (1) {
        ...
        //заснуть, если количество занятых кабинетов равно 3 (никому заходить нельзя, ждем команды админов)
        while (count == 3) {
        ...
        rear = (rear+1)%bufSize ;
        count++ ; //занятая ячейка стала занята

        //конец критической секции

        pthread_mutex_unlock(&mutex) ;
        //разбудить потоки-администраторы после получения элемента из буфера
        pthread_cond_broadcast(&not_full) ;
        if ((cab[rear%bufSize] == -1) && (cab[(rear+1)%bufSize] != cNum) && (cab[(rear+2)%bufSize] != cNum)) {
            // count-- ;
            cab[rear] = cNum;
            printf("Patient %d: ENTERED cabinet [%d]\n", cNum, rear + 1); // пациент заходит, тк кабинет свободен
            sleep(3);
        }
        //sleep(3);
    }
